...
export function byte2Readable(byte:number, prefix: string = ''):string {
  // 1kb is 1024 bytes
  const kb = 1024;
  const mb = kb * 1024;
  const gb = mb * 1024;
...
// Use any to avoid complex type checking, or to use the same types as GeneralCallback to be precise.
